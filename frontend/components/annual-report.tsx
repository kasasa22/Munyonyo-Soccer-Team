...
export function AnnualReport() {
  ...
  // Fetch annual subscription data
  const fetchAnnualData = async () => {
    ...
    try {
      ...
      // Fetch all payments using the same approach as monthly report
      ...
      console.log(`🔍 DEBUG: Fetched payments for ${selectedYear}:`, paymentsData)
      console.log(`🔍 DEBUG: API response status:`, paymentsResponse.status, paymentsResponse.ok)

      // Build annual report data
      const reportData: AnnualReportData[] = []

      for (const player of playersData) {
        // Calculate carryover (only for 2025+)
        const carryover = await calculateCarryover(player.id, selectedYear)
        
        // Calculate current year payments (filter by year, player ID, and payment type)
        ...
        // Debug: Log player payment matching
        if (playerPayments.length > 0) {
          console.log(`✅ ${player.name} has ${playerPayments.length} payments:`, playerPayments)
        } else {
          console.log(`❌ ${player.name} (ID: ${player.id}) has no matching payments`)
          // Check if any payments have similar IDs
          const allPlayerIds = paymentsData.map((p: any) => p.player_id)
          console.log(`Available payment player IDs:`, [...new Set(allPlayerIds)])
        }

        const amountPaid = playerPayments.reduce((sum: number, payment: any) => sum + payment.amount, 0)
        
        // Calculate total amount (base + carryover for 2026+)
        const baseAmount = player.annual
        const totalAmount = parseInt(selectedYear) >= 2026 ? baseAmount + carryover : baseAmount
        const balance = Math.max(0, totalAmount - amountPaid)
        
        // Find last payment date
        const lastPayment = playerPayments.length > 0 
          ? new Date(Math.max(...playerPayments.map((p: any) => new Date(p.date).getTime()))).toLocaleDateString()
          : "-"
        ...
        if (parseInt(selectedYear) >= 2026) {
          reportEntry.carryover = carryover
        }
