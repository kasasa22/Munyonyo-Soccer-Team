...
export function DailyPaymentReport(){
  ...
  const generateDailyReport = async (date: string) => {
    try {
      ...
      const uniquePlayers = new Set(dailyPayments.map(payment => payment.player_id)).size
      ...
      dailyPayments.forEach(payment => {
        if (payment.payment_type in paymentsByType) {
          paymentsByType[payment.payment_type as keyof typeof paymentsByType].count++
          paymentsByType[payment.payment_type as keyof typeof paymentsByType].amount += payment.amount
        }
      })
            ...
            <tbody>
              ${filteredPayments.map(payment => `
                <tr>
                  ...
                  <td><strong>${payment.player_name}</strong></td>
